  ...
  return (
    <div style={{ maxWidth: 960, margin: '0 auto' }} className="animate-fade-in-up">
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, paddingBottom: 16, borderBottom: `1px solid ${C.outlineVariant}30` }}>
        <Link to="/dashboard/history" style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14, color: C.onSurfaceVariant, textDecoration: 'none', transition: 'color 0.15s' }}>
          <ArrowLeft style={{ width: 16, height: 16 }} /> Back to History
        </Link>
        <button onClick={handleDelete} disabled={deleting}
          style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 8, fontSize: 14, color: '#dc2626', background: 'none', border: 'none', cursor: 'pointer', transition: 'all 0.15s', opacity: deleting ? 0.5 : 1, fontFamily: 'inherit' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {metaItems.map((item, i) => (
              <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 0', borderBottom: i < metaItems.length - 1 ? `1px solid ${C.outlineVariant}20` : 'none' }}>
                ...
                <div>
                  <p style={{ fontSize: 12, color: C.onSurfaceVariant }}>{item.label}</p>
                  <p style={{ fontSize: 14, fontWeight: 500, color: C.primary, marginTop: 1 }}>{item.value}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="lg:col-span-2">
          ...
          <div style={{ paddingBottom: 20, borderBottom: `1px solid ${C.outlineVariant}30` }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 12, marginBottom: 12 }}>
              <h2 style={{ fontFamily: "'Manrope', sans-serif", fontSize: 24, fontWeight: 700, color: C.primary, margin: 0 }}>{diagnosis.diseaseName}</h2>
            ...
            <p style={{ fontSize: 15, color: C.onSurfaceVariant, lineHeight: 1.7 }}>{diagnosis.description}</p>
          </div>

          {/* Symptoms */}
          <div style={{ padding: '20px 0', borderBottom: `1px solid ${C.outlineVariant}30` }}>
            <h3 style={{ fontSize: 16, fontWeight: 600, color: C.primary, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2" style={{ gap: 8 }}>
              {diagnosis.symptoms.map((s, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0' }}>
                  ...
                  <span style={{ fontSize: 15, color: C.onSurfaceVariant }}>{s}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Treatment */}
          <div style={{ padding: '20px 0', borderBottom: `1px solid ${C.outlineVariant}30` }}>
            <h3 style={{ fontSize: 16, fontWeight: 600, color: C.primary, marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
              <CheckCircle2 style={{ width: 16, height: 16, color: '#059669' }} /> Treatment Recommendations
            </h3>
            <p style={{ fontSize: 15, color: C.onSurfaceVariant, lineHeight: 1.7 }}>{diagnosis.treatment}</p>
          </div>

          {/* Notes */}
          {diagnosis.notes && (
            <div style={{ padding: '20px 0', borderBottom: `1px solid ${C.outlineVariant}30` }}>
              <h3 style={{ fontSize: 16, fontWeight: 600, color: C.primary, marginBottom: 8 }}>Notes</h3>
              <p style={{ fontSize: 15, color: C.onSurfaceVariant, lineHeight: 1.7 }}>{diagnosis.notes}</p>
            </div>
          )}

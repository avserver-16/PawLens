    ...
    return (
      <div className="animate-fade-in-up" style={{ maxWidth: 900 }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <h2 style={{ fontFamily: "'Manrope', sans-serif", fontSize: 22, fontWeight: 700, color: C.primary }}>Analysis Complete</h2>
          <p style={{ fontSize: 14, color: C.onSurfaceVariant, marginTop: 4 }}>Here are the results</p>
        </div>
          ...
          <div>
            ...
            {result.petName && (
              <div style={{ padding: '16px 0', borderBottom: `1px solid ${C.outlineVariant}30` }}>
                <p style={{ fontSize: 12, color: C.onSurfaceVariant }}>Pet Name</p>
                <p style={{ fontSize: 14, fontWeight: 600, color: C.primary, marginTop: 2 }}>{result.petName}</p>
              </div>
            )}
          </div>
          ...
          <div className="lg:col-span-2" style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
            ...
            <div style={{ paddingBottom: 20, borderBottom: `1px solid ${C.outlineVariant}30` }}>
              <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 12 }}>
                <div>
                  <p style={{ fontSize: 12, color: C.onSurfaceVariant }}>Diagnosis</p>
                  <h3 style={{ fontFamily: "'Manrope', sans-serif", fontSize: 22, fontWeight: 700, color: C.primary, marginTop: 2 }}>{result.diseaseName}</h3>
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  <span style={{ padding: '4px 12px', borderRadius: 8, fontSize: 13, fontWeight: 600, background: sev.bg, color: sev.color, border: `1px solid ${sev.border}` }}>
                    {result.severity}
                  </span>
                  <span style={{ padding: '4px 12px', borderRadius: 8, fontSize: 13, fontWeight: 600, background: `${C.secondary}12`, color: C.secondary }}>
                    {result.confidence}% Match
                  </span>
                </div>
              </div>
              <p style={{ fontSize: 15, color: C.onSurfaceVariant, lineHeight: 1.7 }}>{result.description}</p>
            </div>

            {/* Symptoms */}
            <div style={{ padding: '20px 0', borderBottom: `1px solid ${C.outlineVariant}30` }}>
              <h4 style={{ fontSize: 14, fontWeight: 600, color: C.primary, marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
                <AlertTriangle style={{ width: 16, height: 16, color: '#d97706' }} /> Symptoms
              </h4>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {result.symptoms.map((s, i) => (
                  <span key={i} style={{ padding: '4px 12px', borderRadius: 8, background: C.surfaceContainerLow, border: `1px solid ${C.outlineVariant}40`, fontSize: 13, fontWeight: 500, color: C.onSurfaceVariant }}>{s}</span>
                ))}
              </div>
            </div>
            ...
            <div style={{ padding: '20px 0', borderBottom: `1px solid ${C.outlineVariant}30` }}>
              ...
              <p style={{ fontSize: 15, color: C.onSurfaceVariant, lineHeight: 1.7 }}>{result.treatment}</p>
            </div>

            <div style={{ display: 'flex', gap: 12, paddingTop: 20 }}>
              <button onClick={resetForm} style={{
                display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 14, padding: '10px 20px',
  ...
  return (
    <div className="animate-fade-in-up" style={{ maxWidth: 680 }}>
      <div style={{ marginBottom: 28, paddingBottom: 20, borderBottom: `1px solid ${C.outlineVariant}40` }}>
        <h2 style={{ fontFamily: "'Manrope', sans-serif", fontSize: 22, fontWeight: 700, color: C.primary, margin: 0 }}>New Skin Scan</h2>
        <p style={{ fontSize: 14, color: C.onSurfaceVariant, marginTop: 4 }}>Upload a photo for AI-powered analysis</p>
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        {error && (
          <div style={{ padding: 12, borderRadius: 12, background: '#fef2f2', border: '1px solid #fecaca', color: '#dc2626', fontSize: 14 }} className="animate-fade-in">{error}</div>
        )}
        ...
        >
          {preview ? (
            ...
            <div style={{ padding: '48px 24px', textAlign: 'center' }} onClick={() => fileInputRef.current?.click()}>
              ...
              <p style={{ fontSize: 16, fontWeight: 600, color: C.primary, marginBottom: 4 }}>Drop image here</p>
              <p style={{ fontSize: 14, color: C.onSurfaceVariant, marginBottom: 12 }}>or click to browse</p>
              <p style={{ fontSize: 13, color: C.outlineVariant }}>JPEG, PNG, WebP · Max 10MB</p>
            </div>
          )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2" style={{ gap: 16 }}>
          <div>
            <label style={{ display: 'block', fontSize: 14, fontWeight: 500, color: C.primary, marginBottom: 6 }}>
        ...
        <div style={{ padding: 20, borderTop: `1px solid ${C.outlineVariant}30` }}>
          ...
          <div className="grid grid-cols-2" style={{ gap: 8 }}>
            {['Good natural lighting', 'Focus on affected area', 'Clear, sharp image', 'Multiple angles help'].map(tip => (
              <div key={tip} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: C.onSurfaceVariant }}>

    ...
    <div className="animate-fade-in-up" style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4" style={{ borderBottom: `1px solid ${C.outlineVariant}40`, paddingBottom: 24 }}>
        {statCards.map((s, i) => (
          <div key={i} style={{ padding: '0 16px', borderRight: i < 3 ? `1px solid ${C.outlineVariant}30` : 'none' }}>
            ...
            <p style={{ fontSize: 14, fontWeight: 500, color: C.onSurfaceVariant, marginTop: 4 }}>{s.label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3" style={{ gap: 24 }}>
        ...
        <div className="lg:col-span-2" style={{ paddingBottom: 24, borderBottom: `1px solid ${C.outlineVariant}40` }}>
          <h3 style={{ fontSize: 16, fontWeight: 600, color: C.primary, marginBottom: 20 }}>Disease Distribution</h3>
          {diseaseDistribution.length > 0 ? (
                ...
                return (
                  <div key={d._id}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                      <span style={{ fontSize: 15, fontWeight: 500, color: C.primary }}>{d._id}</span>
                      <span style={{ fontSize: 13, color: C.onSurfaceVariant }}>{d.count} · {pct}%</span>
                    </div>
            ...
            <div style={{ textAlign: 'center', padding: '48px 0' }}>
              ...
              <p style={{ fontSize: 15, color: C.onSurfaceVariant, marginBottom: 16 }}>No scans yet</p>
        ...
        <div style={{ paddingBottom: 24, borderBottom: `1px solid ${C.outlineVariant}40` }}>
          <h3 style={{ fontSize: 16, fontWeight: 600, color: C.primary, marginBottom: 20 }}>Severity Overview</h3>
                ...
                return (
                  <div key={sev} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0', borderBottom: `1px solid ${C.outlineVariant}20` }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <span style={{ fontSize: 14, fontWeight: 500, color: config.color }}>{sev}</span>
                    </div>
                    <span style={{ fontSize: 14, fontWeight: 700, color: config.color }}>{count}</span>
                  </div>
                );
              })}
            </div>
          ) : (
            <p style={{ fontSize: 15, color: C.onSurfaceVariant, textAlign: 'center', padding: '32px 0' }}>No data yet</p>
          )}
        </div>
      </div>
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          <h3 style={{ fontSize: 16, fontWeight: 600, color: C.primary }}>Recent Scans</h3>
          {recentScans.length > 0 && (
            <Link to="/dashboard/history" style={{ fontSize: 13, fontWeight: 600, color: C.secondary, textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 4 }}>
        ...
        {recentScans.length > 0 ? (
              ...
              return (
                <Link key={scan._id} to={`/dashboard/diagnosis/${scan._id}`}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: 15, fontWeight: 500, color: C.primary, margin: 0 }}>{scan.diseaseName}</p>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
                      <span style={{ fontSize: 12, padding: '2px 8px', borderRadius: 100, fontWeight: 500, background: config.bg, color: config.color }}>
                        {scan.severity}
                      </span>
          ...
          <p style={{ fontSize: 15, color: C.onSurfaceVariant, textAlign: 'center', padding: '32px 0' }}>No scans yet. Upload your first image to get started.</p>
        )}
      </div>

  ...
  return (
    <div className="animate-fade-in-up" style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16, paddingBottom: 20, borderBottom: `1px solid ${C.outlineVariant}40` }}>
        <div>
          <h2 style={{ fontFamily: "'Manrope', sans-serif", fontSize: 20, fontWeight: 700, color: C.primary, margin: 0 }}>Scan History</h2>
          <p style={{ fontSize: 13, color: C.onSurfaceVariant, marginTop: 4 }}>{pagination.total} total scans</p>
        </div>
        <div style={{ position: 'relative', width: 260 }}>
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
            style={{ width: '100%', paddingLeft: 40, padding: '10px 14px 10px 40px', borderRadius: 10, border: `1px solid ${C.outlineVariant}`, background: C.surface, fontSize: 13, outline: 'none', fontFamily: 'inherit', color: C.onSurface, boxSizing: 'border-box' }}
      ...
      {filtered.length > 0 ? (
                ...
                <div key={d._id} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '16px 0', borderBottom: i < filtered.length - 1 ? `1px solid ${C.outlineVariant}25` : 'none' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                      <h3 style={{ fontSize: 14, fontWeight: 600, color: C.primary, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.diseaseName}</h3>
                      {d.petName && <span style={{ fontSize: 12, color: C.onSurfaceVariant }}>· {d.petName}</span>}
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, color: C.onSurfaceVariant }}>
                      <span style={{ padding: '2px 8px', borderRadius: 6, fontSize: 11, fontWeight: 600, background: config.bg, color: config.color }}>{d.severity}</span>
                      <Calendar style={{ width: 12, height: 12 }} />
                      {new Date(d.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}
                      <span style={{ padding: '2px 6px', borderRadius: 4, background: `${C.secondary}15`, color: C.secondary, fontWeight: 600 }}>{d.confidence}%</span>
                    </div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                    <Link to={`/dashboard/diagnosis/${d._id}`}
                      style={{ padding: '6px 12px', background: `${C.secondary}12`, color: C.primary, borderRadius: 8, fontSize: 12, fontWeight: 600, textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 4, transition: 'background 0.15s' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
              ...
              {Array.from({ length: pagination.pages }, (_, i) => i + 1).map(p => (
                <button key={p} onClick={() => loadDiagnoses(p)}
                  style={{
                    width: 36, height: 36, borderRadius: 8, fontSize: 13, fontWeight: 500, cursor: 'pointer', transition: 'all 0.15s', fontFamily: 'inherit',
        ...
        <div style={{ textAlign: 'center', padding: '64px 0' }}>
          ...
          <h3 style={{ fontSize: 16, fontWeight: 600, color: C.primary, marginBottom: 4 }}>{search ? 'No results found' : 'No scans yet'}</h3>
          <p style={{ fontSize: 14, color: C.onSurfaceVariant, marginBottom: 20 }}>{search ? 'Try another search.' : 'Upload your first image to get started.'}</p>
          {!search && (

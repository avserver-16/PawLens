...
export default function RegisterPage() {
  ...
  const pwColors = [C.outlineVariant, '#f87171', '#fbbf24', '#34d399', '#22c55e'];
      ...
      <div className="hidden lg:flex" style={{
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          <h2 style={{ fontFamily: "'Manrope', sans-serif", fontSize: 28, fontWeight: 700, color: '#fff', lineHeight: 1.3, marginBottom: 16 }}>
            Join Thousands of<br />Pet Owners
          </h2>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '48px 24px', background: C.surfaceContainerLow }}>
        <div style={{ width: '100%', maxWidth: 400 }} className="animate-fade-in-up">
          ...
          <div style={{ marginBottom: 32 }}>
            <h1 style={{ fontFamily: "'Manrope', sans-serif", fontSize: 24, fontWeight: 700, color: C.primary, marginBottom: 8 }}>Create your account</h1>
            <p style={{ fontSize: 14, color: C.onSurfaceVariant }}>Get started with PawLens in seconds.</p>
          </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 500, color: C.primary, marginBottom: 6 }}>Username</label>
              <div style={{ position: 'relative' }}>
                <User size={16} style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', color: C.outlineVariant }} />
                <input type="text" name="username" value={formData.username} onChange={handleChange} required minLength={3} maxLength={30} style={inputStyle} placeholder="pawlover123" />
              </div>
            </div>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <input type={showPassword ? 'text' : 'password'} name="password" value={formData.password} onChange={handleChange} required minLength={6} style={{ ...inputStyle, paddingRight: 40 }} placeholder="Min 6 characters" />
              ...
              {formData.password && (
                <div style={{ display: 'flex', gap: 4, marginTop: 8 }}>
                  {[1, 2, 3, 4].map(i => (
                    <div key={i} style={{ height: 4, flex: 1, borderRadius: 100, background: i <= pwStrength ? pwColors[pwStrength] : C.outlineVariant, transition: 'all 0.3s' }} />
                  ))}
                </div>
              )}
            </div>
            ...
            <button type="submit" disabled={loading} style={{
              ...
              {loading ? <div style={{ width: 16, height: 16, border: '2px solid rgba(255,255,255,0.3)', borderTop: '2px solid #fff', borderRadius: '50%', animation: 'spin 1s linear infinite' }} /> : <>Create Account <ArrowRight size={16} /></>}
            </button>
          </form>

          <p style={{ marginTop: 24, textAlign: 'center', fontSize: 14, color: C.onSurfaceVariant }}>
            Already have an account?{' '}
            <Link to="/login" style={{ color: C.secondary, fontWeight: 600, textDecoration: 'none' }}>Sign in</Link>
          </p>
        </div>
      </div>
